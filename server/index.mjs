 ...
 *
 * По умолчанию только локальный. Служба не рассчитана на то, чтобы стоять
 * лицом к интернету: перед ней должен быть веб-сервер, который завершает
 * HTTPS. Слушая все интерфейсы, она оказалась бы на публичном адресе сервера —
 * открытым переходником, через который кто угодно ходит по чужим ссылкам от
 * вашего имени.
 *
 * В контейнере переопределяется на 0.0.0.0: там «все интерфейсы» — это сеть
 * контейнера, наружу она не выходит.
 */
...
server.listen(PORT, HOST, () => {
  console.log(`служба разбора рецептов слушает ${HOST}:${PORT}`)
  if (ORIGINS.length) console.log(`разрешённые сайты: ${ORIGINS.join(', ')}`)
})
