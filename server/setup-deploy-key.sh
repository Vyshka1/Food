...
# Проверяем только свой файл. `visudo -c` смотрит разом на все, и падает, если
# у соседнего проекта на этой машине неверные права, — к нашему правилу это
# отношения не имеет, а установку останавливало.
visudo -cf /etc/sudoers.d/food-deploy >/dev/null || fail "sudoers не принял правило"

# А про соседние всё-таки скажем: sudo такой файл молча не читает, и правило,
# которое кто-то там завёл, просто не работает.
BAD="$(visudo -c 2>&1 | grep 'bad permissions' || true)"
if [ -n "$BAD" ]; then
  echo
  echo "   Между прочим, на этой машине есть чужие файлы sudoers с неверными правами:"
  echo "$BAD" | sed 's/^/     /'
  echo "     sudo их не читает, то есть заведённые в них правила не работают."
  echo "     Чинится так: sudo chmod 440 ПУТЬ — но это включит правило, которое"
  echo "     сейчас не действует, так что сначала посмотрите, что в нём."
fi
