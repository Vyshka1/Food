...
export interface Recipe {
  ...
  batch?: RecipeBatch
  /**
   * Как блюдо подготавливают: целиком, заготовкой, сборкой перед едой или
   * просто раскладыванием. Пусто — `cookComplete`: самое обычное поведение.
   * Проставляется вручную там, где оно другое; см. lib/cookMode.
   */
 ...
 */
/** Как штучное блюдо готовится партиями. */
export interface PieceCooking {
  /** Сколько изделий помещается за один заход: сковорода, противень, форма. */
  perLoad: number
  /** Сколько минут занимает один заход. */
  loadMinutes: number
  /** Удобные размеры готовки: человек делает 12 или 16 оладий, а не 13. */
  sizes: number[]
  /** Больше этого за раз не делают, сколько бы ни было едоков. */
  max: number
}
...
export interface RecipeBatch {
  ...
  yieldGrams: number
  /**
   * Как это жарится и какие количества человек считает нормальными.
   *
   * Без этого «33 оладьи» — честное число по составу и неправда по кухне: на
   * сковороде помещается пять, значит это семь заходов и полчаса у плиты, а
   * время бралось из базового рецепта с поправкой не больше чем вдвое.
   */
  piece?: PieceCooking
...
/**
 * Чем определяется количество. Первая из двух независимых осей.
 *
 * Это не новый словарь рядом с `BatchReason`, а его огрубление: причина
 * отвечает «почему партия такого размера», политика — «бывает ли у этого блюда
 * партия вообще и чем она меряется». `fresh` — это `exactForMenu`, изделия —
 * `discretePieces`, всё остальное — `fixedBatch`. Считается по самой партии
 * (lib/cookMode), нигде не хранится и потому не может с ней разойтись.
 *
 * Значение по умолчанию — `exactForMenu`: партия бывает только у блюд, для
 * которых её кто-то подтвердил вручную (см. data/verifiedBatches).
 */
export type BatchPolicy =
  /** Готовим ровно потребность меню. Так по умолчанию у любого блюда. */
  | 'exactForMenu'
  /** Подтверждённая партия: кастрюля, форма, упаковка главного продукта. */
  | 'fixedBatch'
  /** Подтверждённая партия в штуках: котлеты, сырники, маффины, голубцы. */
  | 'discretePieces'

/**
 * Как блюдо подготавливают. Вторая ось, независимая от первой.
 *
 * Оси независимы не на словах: боул с курицей и киноа готовит компоненты
 * партией (`fixedBatch`), а собирается перед едой (`assembleBeforeEating`);
 * чиа-пудинг готовится целиком заранее (`cookComplete`), но ровно на
 * запланированные порции (`exactForMenu`).
 *
 * Хранится у рецепта (`Recipe.preparation`). Не выводится по тексту шагов:
 * правило по тексту полезно как аудит — оно подсказывает, кому проставить
 * значение, — но источником истины быть не должно.
 */
export type PreparationMode =
  /** Блюдо доводят до готового вида целиком: суп, котлеты, чиа-пудинг. */
  | 'cookComplete'
  /** Готовят заготовку и убирают на хранение: хумус, соус, шарики. */
  | 'prepareComponents'
  /** Части готовы, блюдо собирают перед едой: йогурт, боул, тост, салат. */
  | 'assembleBeforeEating'
  /** Приготовления нет вовсе: нарезать и разложить по порциям. */
  | 'portionOnly'
...
export type BatchReason =
  ...
  | 'keeps'
  /**
   * Партии нет: готовим ровно потребность меню. Так у всех трёх режимов, где
   * впрок не делают, — assembleOnDemand, componentPrep, portionOnly.
   */
  | 'fresh'
...
export interface MenuEntry {
  ...
  storage: Storage
  /**
   * Блюдо не готовится на этой неделе: оно уже лежит в морозилке с прошлой.
   * Такую запись не нужно ни готовить, ни покупать — только достать заранее.
   */
  fromFreezer?: boolean
...
/**
 * Что случилось с этим приёмом пищи. Только про тарелку.
 *
 * Готовки здесь нет намеренно. Одна готовка закрывает несколько приёмов и
 * несколько человек, поэтому продукты списываются один раз на всю готовку, а
 * не на каждую запись меню: иначе «приготовлено», нажатое на понедельник и на
 * вторник, списало бы продукты дважды и дважды положило бы остаток в
 * морозилку. Факт готовки живёт в CookEvent.
 */
export type EntryStatus = 'eaten' | 'skipped'

export const ENTRY_STATUS: { id: EntryStatus; label: string; icon: 'check' | 'ban' }[] = [
 ...
 *
 * Это снимок, а не ссылка на план. План пересчитывается — от изменения
 * кладовой, замены блюда, новой недели, — а списанное уже списано. Пока факт
 * выводился из плана, отметка «приготовлено» меняла смысл задним числом: план
 * пересчитался, и в кладовой оказывалось не то, что из неё брали.
 */
export interface CookEvent {
  /** Устойчивый ключ готовки: неделя, блюдо, день готовки. */
  taskId: string
  /** Когда отметили, ISO-дата. */
  at: string
  recipeId: string
  /** Сколько долей рецепта поставили на плиту. */
  servings: number
  /** Сколько готового получилось, г. */
  cookedGrams: number
  /** Что списали из кладовой. Постоянных продуктов здесь нет — их не считают. */
  used: { ingredientId: string; qty: number }[]
  /** Что убрали в морозилку, если убирали. */
  frozen?: { containers: number; portionsEach: number; grams: number }
}
...
/**
 * Партия впрок: порции сверх недельной потребности, которые замораживают на
 * начало следующей недели.
 *
 * Готовка идёт витками, а неделя — календарь. Если первая готовка недели не в
 * понедельник, то дни до неё кормит не эта неделя, а прошлая: раньше их
 * закрывала молча дописанная понедельничная готовка, и «готовим по
 * воскресеньям» на деле означало «и ещё немного в понедельник».
 *
 * Запас живёт здесь, а не отдельной записью меню, ровно по одной причине: у
 * MenuEntry нет способа сказать «съедят на следующей неделе». День и день
 * готовки у неё — числа 0..6 внутри своей недели, и запись с чужой недели
 * привязалась бы к чужой готовке: кнопка «Приготовлено» списала бы продукты
 * под ещё не случившуюся варку.
 */
export interface AheadBatch {
  recipeId: string
  /** День недели, когда эту партию готовят (индекс внутри своей недели). */
  cookDay: number
  slot: MealSlot
  /** Порций сверх того, что съедят на этой неделе. */
  portions: number
  /** Дни следующей недели, ради которых запас и варится. */
  forDays: number[]
}
...
export interface WeekMenu {
  ...
  entries: MenuEntry[]
  /**
   * Что варится впрок. Необязательно: у недель, собранных до появления
   * запаса, и у семей без морозилки его нет.
   */
  ahead?: AheadBatch[]
}
...
export interface FreezerItem {
  ...
  keepDays: number
  /**
   * КБЖУ и цена одной порции той партии, из которой заготовка.
   *
   * Заготовку едят через недели, когда плана той готовки давно нет, а
   * калории у неё те же, что были в кастрюле: с досыпанным остатком упаковки
   * и целой луковицей вместо половины. У заготовок, замороженных до появления
   * этого поля, его нет — тогда считаем по составу рецепта.
   */
  stats?: Norms & { price: number }
}
...
/**
 * Всё, что приложение хранит между запусками.
 *
 * Это корень сохраняемых данных, поэтому он живёт рядом с остальными типами, а
 * не внутри React-файла: разбор и миграции — работа чистая, и проверять её надо
 * без экрана.
 */
export interface AppState {
   ...
   *
   * Нужна, чтобы старая сборка из кэша узнала данные новой и отказалась их
   * переписывать. Значение задаёт `lib/persist`.
   */
  version: number
   ...
   *
   * Отметка недельная, как и `bought`: она про список этой недели и вместе с
   * неделей кончается (см. `rotateWeek`). То, что есть дома дольше недели, —
   * это `pantry`: «есть всегда» или количество в запасах.
   */
  atHome: string[]
  ...
  /** Что человек отметил купленным в списке этой недели. */
  bought: string[]
  /**
   * Строки покупки, излишек которых уже разложен по кладовой.
   *
   * Это не второй ответ на вопрос «куплено ли»: на него отвечает `bought`.
   * Здесь — что приложение уже сделало с кладовой, чтобы не сделать это
   * дважды. Пачка фарша 500 г при нужных 275 кладёт в запасы 225 г ровно один
   * раз; второе нажатие «Разложить покупки» не должно надувать запасы из
   * воздуха.
   */
  stored: string[]
  warnings: string[]
