 ...
 *
 * Это единственный источник производственных партий в приложении. Блюдо,
 * которого здесь нет, готовится ровно на потребность меню — и это умолчание
 * намеренно безопасное: ошибка «приготовили сколько съедят» стоит одной лишней
 * готовки, а ошибка «вывели партию по тексту шагов» стоила килограмма йогурта
 * с размокшей гранолой на два завтрака по 285 г.
 *
 * Раньше партию выводило правило в lib/batchInfo: якорный продукт задавал
 * закладку по упаковке, а если якоря не было — кастрюля в четыре доли. Оно
 * назначало партию 44 блюдам из 78, и пяти из них — зря. Теперь оно живёт
 * только как аудит (`looksBatchableRecipe`), и тест сверяет этот список с ним:
 * новое блюдо не получит партию молча, но и не потеряется.
 *
 ...
 *
 * Если после готовки окажется, что изделий выходит другое число, менять нужно
 * здесь: больше этих чисел нигде нет.
 */
...
export const VERIFIED_BATCHES: Record<string, RecipeBatch> = {
  ...
  // Боул: партия здесь про компоненты, а не про блюдо. Киноа и курицу варят
  // пачкой филе и хранят, а собирают боул перед едой — это вторая ось,
  // `preparation: 'assembleBeforeEating'` в recipes.ts.
  ...
  // Бефстроганов: говядина 500 г, 135 г на долю → без малого четыре доли.
  // Блюдо со сметаной, поэтому готовым не морозится; сырой остаток мяса — да.
  beef_stroganoff: {
    ...meatBatch('beef', 3.75, 1215),
    freezeCooked: false,
  },
  ...
  /*
   * Блюда, которые варят кастрюлей, но не морозят: тофу после разморозки
   * становится губкой, картофель — водянистым. Партия у них настоящая, а
   * излишек деть некуда, кроме тарелок, — и цена ошибки в размере выше.
   */
  tofu_stirfry: { ...potBatch(1265), freezeCooked: false },
  potato_tortilla: { ...potBatch(1315), freezeCooked: false },
  /*
   * Хумус: это не блюдо на тарелке, а заготовка в контейнере — её достают
   * ложкой к овощам несколько дней подряд. Блендеру нужен объём: 100 г нута он
   * просто размажет по стенкам, и меньше банки хумус не делают.
   */
  hummus_classic: { ...potBatch(405), freezeCooked: false },
  beet_hummus: { ...potBatch(575), freezeCooked: false },
  ...
  // Запеканки с сыром и сметаной: в морозилке заливка расслаивается, поэтому
  // готовыми их не морозят. Размер задаёт форма, как и у остальных.
  zucchini_casserole: { ...formBatch(3.5, 1230), freezeCooked: false },
  cauliflower_bake: { ...formBatch(3.25, 1225), freezeCooked: false },
  // Печёные яблоки: одно яблоко — одна порция, форма вмещает четыре. Штука
  // здесь очевидная и целая, поэтому число изделий выставлено.
  baked_apple: {
    ...formBatch(4, 725),
    freezeCooked: false,
    yieldPieces: 4,
    pieceName: ['яблоко', 'яблока', 'яблок'],
    piece: { perLoad: 4, loadMinutes: 25, sizes: [2, 4, 6, 8], max: 8 },
  },
