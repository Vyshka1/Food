...
export const CATEGORY_LABEL: Record<IngredientCategory, string> = {
  ...
  /*
   * «Бакалея» — и правда почти всё, что сюда попало: масла, специи, соусы,
   * консервы, а ещё кофе, чай, сок, газировка, пиво, сахар и какао. Одним
   * более узким словом это не накрыть, и попытка была: «Масло, соусы и
   * специи» стояло заголовком над строкой «Пиво светлое, 3,5 л».
   *
   * Совпадение с названием отдела в `lib/stores` — не повод врать здесь.
   * Задвоение «Бакалея / Бакалея» снимается на экране, где оно видно.
   */
  pantry: 'Бакалея',
}
