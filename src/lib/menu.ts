...
/**
 * Сообщение о дописанной понедельничной готовке.
 *
 * Названо константой, потому что его читают в двух местах: собирает его
 * подбор меню, а показывает — экран готовки. На экране меню ему не место:
 * человек смотрит пятницу, а речь про понедельник, и это к тому же не
 * предупреждение, а отчёт о уже сделанном. Сверять два экрана по тексту
 * нельзя — поэтому текст один и лежит здесь.
 */
export const IMPLICIT_COOK_NOTE =
  'Понедельник не отмечен днём готовки — добавили короткую готовку, чтобы закрыть начало недели.'
 ...
 *
 * Перемерено после того, как потребность переехала в готовый вес, а дневной
 * итог стал считаться по фактически приготовленным партиям. Замер на 280 днях
 * при норме 26 г и на 12 неделях × 12 прогонов — дней ниже нормы / худший
 * день / касса ₽/нед:
 *
 *   вес        0     12     16     20
 *   дней      44     32     23     20
 *   худший    13     21     21     21
 *   касса      —   6966   6975   7065
 *
 * Двенадцать, стоявшие здесь раньше, оставляли 32 дня из 280 — при том, что
 * проверка требует меньше 12% (33.6 дня). Полтора дня запаса это не запас.
 * Шестнадцать сокращают провалы на треть и стоят девять рублей в неделю, то
 * есть ничего. Двадцать снимают ещё три дня, но уже за сотню рублей.
 *
 * Цена всё же есть, и она не в деньгах: чем тяжелее клетчатка, тем меньше
 * остаётся цели пересборки «разнообразнее» — подбор и так тянется к бобовым и
 * овощам.
 */
const FIBER_PER_1000 = 14
const FIBER_WEIGHT = 16
 ...
 *
 * Прежнее обоснование было такое: на 0.15 и выше один день из 1680 уходил
 * больше чем на 12% от личной нормы, и обещание «каждому по его норме» дороже
 * процента излишка. Этого обрыва больше нет — он держался на завышенной
 * потребности, которая считалась в сыром весе. Перемер на 12 неделях × 12
 * прогонов (касса ₽/нед, худшее отклонение дня, новых блюд во второй неделе
 * подряд, разных блюд за 12 недель):
 *
 *   0.1 — 6966, 1.5%, 2.9, 23
 *   0.2 — 6698, 1.5%, 2.5, 23
 *   0.3 — 6468, 1.5%, 2.2, 23
 ...
 *   2.0 — 5787, 4.2%, ..,  18
 *
 * То есть теперь это не предохранитель, а развилка: до 0.5 норма не страдает
 * вовсе, а полтысячи рублей в неделю покупаются тем, что соседние недели
 * становятся похожее — новых блюд во второй неделе 2.2 вместо 2.9. Репертуар
 * за двенадцать недель при этом не сужается (23 блюда и там, и там).
 *
 * Оставлено 0.1: выбор между деньгами и новизной — не вопрос замера.
 */
