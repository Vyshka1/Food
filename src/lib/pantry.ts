 ...
 *
 * Отсюда и разделение обязанностей. Галочка `atHome` осталась тем, чем была, —
 * пометкой на списке этой недели, и вместе с неделей она кончается
 * (`rotateWeek`). Всё, что должно пережить понедельник, живёт здесь: «есть
 * всегда» — это `always`, «лежит столько-то» — количество в `stock`. Вечной
 * галочке места нет ни там, ни там: она обещала бы, что дома есть сколько
 * угодно и навсегда, а отмечали один кабачок и на одну неделю.
 *
