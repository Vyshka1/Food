...
describe('что станет с упаковками', () => {
  it('называет блюда, в которые уйдёт остальное', () => {
    ...
    for (const card of allCards()) {
      for (const line of card.leftovers) {
        ...
        checked += 1
        if (line.elsewhere.length > 0) named += 1
    ...
    /*
     * И главное: имена там действительно есть. Без этой проверки тест был
     * пустым — обе проверки внутри цикла верны на пустом списке тождественно,
     * и «в другие блюда» без единого названия прошло бы молча. Замерено: имена
     * названы у 205 строк из 212; остальные семь — продукты, уходящие не в
     * блюда, а в напитки и дополнения к дню, у них называть нечего.
     */
    expect(named / checked).toBeGreaterThan(0.9)
  })
  ...
  /*
   * Калории строк и калории всей готовки должны быть одним числом, разделённым
   * на части, а не двумя ответами на один вопрос.
   *
   * Раньше граммы строки считались от фактически приготовленного, а калории —
   * от доли рецепта. Основания разные: штучное округляется вверх, половину
   * яблока не купишь, и блюдо содержит три яблока там, где рецепт просит два с
   * половиной. Замер на 156 карточках: расхождение до 245 ккал. Человек видел
   * «всей готовки 1631», складывал строки и получал 790.
   *
   * Допуск — 2 ккал на строку: каждая округляется до целого, и это единственное
   * расхождение, которое здесь законно.
   */
  it('калории строк складываются в калории всей готовки', () => {
    const cards = allCards()
    expect(cards.length).toBeGreaterThan(50)
    for (const card of cards) {
      const rowsKcal = card.rows.reduce((sum, r) => sum + r.kcal, 0)
      const rowsGrams = card.rows.reduce((sum, r) => sum + r.grams, 0)
      // на тарелки уходит своя доля готовки — остальное в морозилке и добавке
      const expected = (rowsGrams / Math.max(1, card.cookGrams)) * card.stats.kcal
      expect(Math.abs(rowsKcal - expected)).toBeLessThanOrEqual(2 * card.rows.length)
    }
  })
